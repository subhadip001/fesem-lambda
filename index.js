...
app.post("/fesem/otp", jsonParser, async function (req, res) {
  ...
  return transporter.sendMail(mailOptions4, function (error, info) {
    if (error) {
      console.log(error);
    } else {
...
app.post("/fesem/book", jsonParser, async function (req, res) {
  ...
  if (req.body.userName !== "admin") {
    return transporter.sendMail(mailOptions2, function (error, info) {
      if (error) {
        console.log(error);
      } else {
        console.log("Email sent: " + info.response);
...
app.post("/fesem/admin/delete", jsonParser, async (req, res) => {
  await BookDetails.deleteOne({ _id: req.body.id });
  await User.updateOne(
